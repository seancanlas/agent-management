...
const defaultWriteText = async (path, content) => {
    await mkdir(dirname(path), { recursive: true });
    await writeFileToDisk(path, content, { flag: 'wx' });
};
export function githubBlobToRawUrl(source) {
    let sourceUrl;
    try {
...
function agentNameFrom(fileName, requestedName) {
...
function harnessDirectory(harness, homeDir) {
    if (harness !== 'opencode') {
...
export class AgentManager {
    ...
    async installAgent(source, options = {}) {
        try {
            ...
            const harnesses = [...new Set((options.harnesses ?? ['opencode'])
